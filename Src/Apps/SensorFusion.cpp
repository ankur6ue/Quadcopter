...
//	Serial.println(READ_FS_SEL);
	...
	AccelAngleY = atan2(accX, sqrt(pow(accY,2) + pow(accZ,2)))*RADIANS_TO_DEGREES; // atan(-1*accX/sqrt(pow(accY,2) + pow(accZ,2)))*RADIANS_TO_DEGREES;
	AccelAngleX = atan2(accY, sqrt(pow(accX,2) + pow(accZ,2)))*RADIANS_TO_DEGREES;; // atan(accY/sqrt(pow(accX,2) + pow(accZ,2)))*RADIANS_TO_DEGREES;
	AccelAngleZ = accZ;
	...
	fAngleX = gyroX*dt + fLastGyroAngleX;
	fAngleY = gyroY*dt + fLastGyroAngleY;
	fAngleZ = gyroZ*dt + fLastGyroAngleZ;
#endif
