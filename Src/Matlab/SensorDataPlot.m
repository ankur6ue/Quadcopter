...
function mycallback(obj,event,time)
    val = fscanf(serialObject);
    data = sscanf(val, '%f');
    data
end

count = 1;
serialObject.BytesAvailableFcnMode = 'terminator';
serialObject.BytesAvailableFcn = @mycallback;

userInput = input('enter c to exit', 's');
if (isequal(userInput, 'c'))
